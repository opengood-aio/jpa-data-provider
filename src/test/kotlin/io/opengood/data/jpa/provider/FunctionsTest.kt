...
class FunctionsTest : WordSpec() {
    ...
    init {
        "JPA Data Provider" should {
            ...
            "have nullableObjectFieldValue extension function convert row column map entry and return non-null object field value " +
                "when row contains column entry" {
                    with(dataProvider) {
                        nullableObjectFieldValue("name", mapOf("product_name" to "foo"), convertToString) shouldBe "foo"
                    }
                }

            "have nullableObjectFieldValue extension function not convert row column map entry and return null object field value " +
                "when row does not contain column entry" {
                    with(dataProvider) {
                        nullableObjectFieldValue("name", emptyMap(), convertToString).shouldBeNull()
                    }
                }

            "have nullableObjectFieldValue extension function not convert row column map entry and throws exception when mapping not " +
                "found" {
            ...
            "have nullableRowColumnValue extension function convert object field value and return row column entry when object field " +
                "value is not null" {
                    with(dataProvider) {
                        nullableRowColumnValue("name", "foo", convertToString)
                            ?.shouldBeMapEntry(makeEntry("product_name", "foo"))
                    }
                }

            "have nullableRowColumnValue extension function not convert object field value and return null when object field value is " +
                "null" {
                    with(dataProvider) {
            ...
            "have objectFieldValue extension function convert row column map entry and return non-null object field value when row " +
                "contains column entry" {
                    with(dataProvider) {
                        objectFieldValue("name", mapOf("product_name" to "foo"), "bar", convertToString) shouldBe "foo"
                    }
                }

            "have objectFieldValue extension function not convert row column map entry and return default object field value when row " +
                "does not contain column entry" {
                    with(dataProvider) {
            ...
            "have rowColumnValue extension function convert object field value and return row column entry when object field value is " +
                "not null" {
                    with(dataProvider) {
                        rowColumnValue("name", "foo", "bar", convertToString) shouldBeMapEntry
                            makeEntry("product_name", "foo")
                    }
                }

            "have rowColumnValue extension function not convert object field value and return row column entry with default field value " +
                "when object field value is null" {
                    with(dataProvider) {
                        rowColumnValue("name", null, "bar", convertToString) shouldBeMapEntry
                            makeEntry("product_name", "bar")
                    }
                }
